...
export default function Main({ user }: DetailProps) {
    ...
    const fetchUsers = async () => {
        ...
        try {
            const response = await axios.post<ApiResponse>(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/getuser`, {page});
    ...
    useEffect(() => {
      ...
      const handleConfirmAndCallApi = async (id:string) => {
              ...
              const result = await Swal.fire({
                ...
                text: `ว่าจะลบผู้ใช้นี้ จะไม่สามารถกู้คืนได้?`,
                icon: "warning",
    ...
    return(
        ...
        <div className="flex items-center mb-3 relative">
          <div className="shrink-0 mr-[16px] my-4"><img className="mx-auto md:size-12 size-16 shrink-0 rounded-full" src={u.image}/></div>
            <div className="flex flex-col grow min-w-0 mt-2 relative mt-3">
                <p className="font-bold md:text-md">{u.name}</p>
                <p className="text-gray-600 mt-1 font-semibold"></p>
            </div>
            <div className="absolute right-0 top-0">
                ...
                <button onClick={() => handleConfirmAndCallApi(u.id)} className="inline-flex mr-1 items-center rounded-md bg-white px-2.5 py-1.5 text-sm font-semibold text-gray-900 ring-1 shadow-xs ring-gray-300 ring-inset hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-30 disabled:hover:bg-white">
                    ...
                    <span className="mt-[2px] ml-[12px] text-lg md:block hidden">ลบผู้ใช้</span>
                </button>
